...
# 이미지 기울기 보정을 위한 함수
# 인자값1 : 원본 이미지 / 인자값2 : 얼굴 중심 좌표 / 인자값3 : 양쪽 눈 중심 좌표
def doCorrectionImage(image, face_center, eye_centers):
    ...
    # 이미지를 회전하고 나면 튀어 나오는 부분이 생김, 튀어 나오는 부분 원래 이미지 크기로 자르기
...
# 결과값 : 윗머리, 귀밑머리, 입술, 얼굴 전체
def doDetectObject(face, center):
    w, h = face[2:4]
    center = np.array(center)

    # 얼굴 영역 평균 비율
    # 얼굴 시작 좌표 : 통계학 측면에서 얼굴 중심으로부터 얼굴 높이의 약 65%
    # 얼굴 끝 좌표 : 통계학 측면에서 얼굴 중심으로부터 얼굴 너비의 약 45%
    face_avg_rate = np.multiply((w, h), (0.45, 0.65))

    # 입술 영역 평균 비율
    # 입술 시작 좌표 : 통계학 측면에서 입술 중심으로부터 얼굴 높이의 평균 약 10%
    # 입술 끝 좌표 : 통계학 측면에서 입술 중심으로부터 얼굴 너비의 평균 약 18%
    lip_avg_rate = np.multiply((w, h), (0.18, 0.1))

    # 얼굴 중심에서 머리 시작 좌표로 이동
    pt1 = center - face_avg_rate

    # 얼굴 중심에서 머리 종료 좌표로 이동
...
# 이미지의 관심 영역(ROI, Region of Interest)을 가져오는 함수
# 인자값1: 위치 / 인자값2 : 크기
# 결과값 : 영역 값
def roi(pt, size):
...
# 타원형으로 마스크 생성하는 함수
# 인자값1 : 원본 이미지 / 인자값2 : 세부 영역(윗머리, 귀밑머리, 입술, 얼굴 전체 중 하나)
