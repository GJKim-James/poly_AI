...
unchanged = cv2.imread(image_file, cv2.IMREAD_UNCHANGED)

# OpenCV 색상을 일반적으로 부르는 RGB 순서가 아닌, BGR 순서로 사용함
color = ('b', 'g', 'r')

# 색상이 존재하는 원본 이미지 히스토그램 보기
# BGR 순서대로 그래프 그리기 위해 반복문 사용
for i, col in enumerate(color):
    # calcHist 파라미터 설명
    # 첫 번째 파라미터(images) : 분석할 이미지 파일
    # 두 번째 파라미터(Channel) : 컬러 이미지(BGR)이면, 배열 값 3개로 정의
    # 세 번째 파라미터(Mask) : 분석할 영역의 형태인 mask
    # 네 번째 파라미터(histSize) : 히스토그램의 hist 크기, 예 : 64이면 256/64 = 4 -> 픽셀 4개를 1개의 픽셀로 합쳐서 연산
